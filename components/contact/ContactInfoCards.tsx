...
export default function ContactInfoCards() {
  return (
    <div className="space-y-5 md:space-y-6">
      <article className="bg-[#f8fafc] border border-gray-100 rounded-[24px] p-6 md:p-8">
        ...
        <h3 className="text-lg md:text-xl font-black text-[#002b5c] mb-2">
          Email Inquiry
        </h3>
        <p className="text-gray-500 text-sm md:text-base mb-4 leading-relaxed">
          For non-urgent questions, reports, or general information requests.
        </p>
        <a
          href={contactInfo.emailHref}
          className="text-[#4a90e2] font-bold hover:text-[#002b5c] transition-colors break-all"
        >
          {contactInfo.email}
        </a>
      </article>
      ...
      <article className="bg-[#f8fafc] border border-gray-100 rounded-[24px] p-6 md:p-8">
        ...
        <h3 className="text-lg md:text-xl font-black text-[#002b5c] mb-4">
          Operating Hours
        </h3>
        <ul className="space-y-3">
          {operatingHours.map((item) => (
            ...
            >
              <span className="text-gray-600 font-medium">{item.days}</span>
              <span
                className={`font-bold ${
                  item.highlight ? "text-[#28a745]" : "text-[#002b5c]"
                }`}
              >
                {item.hours}
              </span>
